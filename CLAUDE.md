...
- Adding a *new* export to that layer means touching four files or the API route breaks at runtime with `(0, e.xx) is not a function` (compile stays green): `repos/*.js` → `db/index.js` → the shim list in `localDb.js` → `src/models/index.js`. Routes import `@/models`, and `@/models` re-exports **from `@/lib/localDb`**, not from `db/index.js` directly.
